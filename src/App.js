...
export default function CrickClash() {
  ...
  const handlePostComment = async () => {
    ...
    await set(commentRef, { text: newComment, user: user.displayName, photo: user.photoURL, time: time, likes: {} });
    setNewComment("");
  };
  ...
  useEffect(() => {
    ...
    onValue(commentsRef, (snap) => {
      const data = snap.val();
      const commentsArray = data? Object.values(data).sort((a,b) => b.time - a.time) : [];
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white flex-col">
      ...
      <div className="max-w-md mx-auto w-full flex-1 p-4">
        <header className="flex justify-between items-center mb-4">
          ...
          <img src={user.photoURL} onClick={handleLogout} className="w-10 h-10 rounded-full border-2 border-[#a8ff00] cursor-pointer" />
        </header>
        {weeklyWinner && (
          ...
          <div>
            ...
            <div className="flex items-center justify-center gap-2 mb-4">
              {[battle[0], battle[1]].map(p => (
                <div key={p.id} onClick={() => setSelectedPlayer(p)} className={`bg-[#13131a] p-4 rounded-2xl w-1/2 text-center cursor-pointer hover:scale-105 transition ${voteAnim === p.id? 'vote-pop' : ''}`}>
                  ...
                  <button onClick={(e) => {e.stopPropagation(); handleVote(p.id)}} disabled={isVoting || votesToday >= DAILY_VOTE_LIMIT} className="w-full bg-[#a8ff00] text-black py-3 rounded-xl font-bold mt-2 disabled:bg-gray-600">
                ...
                {comments.map((c) => {
                  ...
                  return (
                    <div key={c.time} className="bg-[#0a0a0f] p-3 rounded-lg">
                      ...
                      <button onClick={() => handleLikeComment(c.time)} className={`flex items-center gap-1 mt-2 text-xs ${isLiked? 'text-[#a8ff00]' : 'text-gray-400'}`}>
                        {isLiked? '❤️' : '🤍'} {likeCount}
                      </button>
                    </div>
                  )
        ...
        {tab === 'Rankings' && (
          <div>{players.sort((a,b) => b.votes - a.votes).slice(0,10).map((p,i) => (<div key={p.id} onClick={() => setSelectedPlayer(p)} className="bg-[#13131a] p-3 rounded-xl mb-3 flex items-center gap-3 cursor-pointer"><span>#{i+1}</span><img src={p.image} className="w-12 h-12 rounded-full" /><span className="font-bold">{p.name}</span><span className="ml-auto text-[#a8ff00]">{p.votes}</span></div>))}</div>
        )}
        {tab === 'History' && (
          <div>{battleHistory.length === 0? <p>No history</p> : battleHistory.map((h,i) => (<div key={i} className="bg-[#13131a] p-3 rounded-xl mb-2"><p>Battle {h.battleNo}: {h.players[0]} vs {h.players[1]}</p><p className="text-[#a8ff00]">Voted: {h.votedFor}</p></div>))}</div>
        )}
      </div>
    </div>
  );
