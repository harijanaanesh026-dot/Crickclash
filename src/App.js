    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <div className="max-w-md mx-auto w-full p-4">
        ...
        {user && (
          <>
            ...
            <div className="bg-[#13131a] p-3 rounded-2xl mb-3">
              ...
              <div className="flex gap-2 flex-wrap">{badges.map(b => <span key={b} className="bg-[#a8ff00] text-black px-3 py-1 rounded-full text-sm font-bold">🏏 {b}</span>)}{badges.length === 0 && <span className="text-gray-500 text-sm">No badges yet</span>}</div>
            </div>
            <div className="bg-[#13131a] p-4 rounded-2xl mb-4 text-center"><p className="text-gray-400 text-sm">Today's Votes Left</p><p className="text-4xl font-bold text-[#a8ff00]">{DAILY_VOTE_LIMIT - votesToday} / {DAILY_VOTE_LIMIT}</p></div>
          </>
        )}
        ...
        {tab === 'Battle' && battle[0] && battle[1] && (
          <div>
            <div className="grid grid-cols-4 text-center mb-6"><div><p className="text-2xl font-bold text-orange-400">{totalVotes}</p><p className="text-xs text-gray-400">TOTAL</p></div><div><p className="text-2xl font-bold text-orange-400">{battleNo-1}</p><p className="text-xs text-gray-400">BATTLES</p></div><div><p className="text-2xl font-bold text-orange-400 truncate">{topPlayer?.name.split(' ')[0] || 'None'}</p><p className="text-xs text-gray-400">TOP</p></div><div><p className="text-2xl font-bold text-orange-400">🔥{streak}</p><p className="text-xs text-gray-400">STREAK</p></div></div>
            ...
            <div className="flex items-center justify-center gap-2">{[battle[0], battle[1]].map(p => <div key={p.id} className="bg-gradient-to-b from-[#1e3a5f] to-[#0a0e1a] p-4 rounded-2xl w-1/2 text-center"><img src={p.image} className="w-20 h-20 rounded-full mx-auto mb-2 object-cover" alt={p.name}/><span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-800">{p.role}</span><h3 className="text-xl font-bold mt-3">{p.name}</h3><p className="text-[#a8ff00] font-bold">{p.votes || 0} votes</p><button onClick={() => handleVote(p.id)} disabled={user && votesToday >= DAILY_VOTE_LIMIT} className="w-full py-3 rounded-xl font-bold mt-2 bg-[#a8ff00] text-black disabled:bg-gray-700">VOTE +10XP</button></div>)}</div>
