...
export default function CrickClash() {
  ...
  const handleVote = async (playerId) => {
    if (!user) { alert("Login chey ra ANESH"); return; }
    ...
    setTimeout(() => {
      if(confirm("Share cheddama?")){
        navigator.share({
          ...
          url: "https://crickclash.vercel.app"
        }).catch(()=>{});
      }
    }, 1000);
  }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400">Created by Anesh 🔥</p>
          </div>
          {user? <button onClick={handleLogout} className="bg-green-500 px-6 py-2 rounded-full font-bold text-black">Logout</button> : <button onClick={handleLogin} className="bg-blue-500 px-6 py-2 rounded-full font-bold">Login</button>}
        </div>
        ...
        <div className="flex justify-around mb-6 border-b border-gray-700">
          ...
          <button className="text-gray-500 font-bold pb-2">🏆 Rankings</button>
          <button className="text-gray-500 font-bold pb-2">📜 History</button>
        </div>
